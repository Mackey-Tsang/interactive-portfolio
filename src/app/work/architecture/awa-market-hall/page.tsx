    ...
    <>
      <main className="min-h-dvh w-full bg-neutral-100 text-neutral-900">
        <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8 py-16 space-y-6">
          ...
          <SectionFadeOnScroll imageUrls={[HERO_RENDER]}>
          ...
          <SectionFadeOnScroll imageUrls={[MASTERPLAN_IMAGE, SECTION_IMAGE, LEGEND_IMAGE]}>
          ...
          {/* 04 — NZ Farmers Market context */}
          <SectionFadeOnScroll imageUrls={FARMERS_MARKET_IMAGES}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[FARMERS_MARKET_IMAGES[0], FARMERS_MARKET_IMAGES[1]].map((src, i) => (
                    <div key={src} className="space-y-2">
                      <button
                        type="button"
                        onClick={() => openLightbox(src, "Farmers market context reference.")}
                        className="block w-full"
                      >
                        <img
                          src={src}
            ...
            <div className="space-y-10">
              ...
              <div className="relative">
                ...
                {PROGRESS_IMAGES[0] && (
                  <button
                    type="button"
                    onClick={() => openLightbox(PROGRESS_IMAGES[0], "Early roof + structure study drawing.")}
                    className="hidden md:block absolute right-6 bottom-[-18%] w-[48%] shadow-[0_18px_45px_rgba(0,0,0,0.35)]"
                  >
                    <img
              ...
              <div className="mt-20 space-y-0 relative">
                {PROGRESS_IMAGES.slice(1).map((src, idx) => {
                  ...
                  return (
                    <div
                      key={src}
                      className={`flex w-full relative ${isEven ? "justify-start" : "justify-end"} ${idx === 0 ? "mt-0" : "-mt-20 sm:-mt-24"}`}
                    >
                      <button
                        onClick={() => openLightbox(src, "Awa Market Hall progress drawing.")}
                        className={`w-[78%] sm:w-[62%] md:w-[48%] shadow-[0_14px_35px_rgba(0,0,0,0.35)] ${isEven ? "ml-4 sm:ml-6 md:ml-8" : "mr-4 sm:mr-6 md:mr-8"}`}
                      >
                        <img
                          src={src}
          ...
          <SectionFadeOnScroll imageUrls={[SECOND_RENDER, FLOORPLAN_IMAGE, ...TRIPLE_16_9_IMAGES, SECTION_RENDER]}>
            <div className="space-y-10">
              <button
                type="button"
                onClick={() => openLightbox(SECOND_RENDER, "Exterior render of Awa Market Hall.")}
                className="block w-full"
              >
                <img
                  src={SECOND_RENDER}
                  alt="Exterior render of Awa Market Hall."
                  className="block w-full h-auto object-cover"
                />
              </button>
              ...
              <button
                type="button"
                onClick={() => openLightbox(FLOORPLAN_IMAGE, "Ground floor plan of Awa Market Hall.")}
                className="block w-full"
              >
                <img
                  src={FLOORPLAN_IMAGE}
              ...
              <div className="grid gap-4 md:grid-cols-3">
                {TRIPLE_16_9_IMAGES.map((src) => (
                  <button
                    key={src}
                    type="button"
                    onClick={() => openLightbox(src, "Perspective vignette.")}
                    className="block w-full"
                  >
                    <div className="aspect-video w-full">
                      <img src={src} alt="Vignette" className="block w-full h-full object-cover" />
                    </div>
                  </button>
                ))}
              </div>
              ...
              <button
                type="button"
                onClick={() => openLightbox(SECTION_RENDER, "Section drawing.")}
                className="block w-full"
              >
                <img src={SECTION_RENDER} alt="Section" className="block w-full h-auto object-cover" />
              </button>
            </div>
          </SectionFadeOnScroll>

          {/* 0Y — Detail Grid 1 */}
          <SectionFadeOnScroll imageUrls={GRID_IMAGES1}>
            <section className="space-y-6">
              <div className="mx-auto w-full max-w-7xl space-y-4">
                {GRID_IMAGES1[0] && (
                  <button
                    type="button"
                    onClick={() => openLightbox(GRID_IMAGES1[0], "Detail study.")}
                    className="block w-full"
                  >
                    <div className="aspect-video w-full overflow-hidden">
                      <img src={GRID_IMAGES1[0]} alt="Detail" className="block w-full h-full object-cover" />
                    </div>
                  </button>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {GRID_IMAGES1.slice(1).map((src) => (
                    <button key={src} type="button" onClick={() => openLightbox(src, "Detail study.")} className="block w-full">
                      <div className="aspect-video w-full overflow-hidden">
                        <img src={src} alt="Detail" className="block w-full h-full object-cover" />
                      </div>
                    </button>
                  ))}
                </div>
          ...
          {/* 0Z — Detail Grid 2 */}
          <SectionFadeOnScroll imageUrls={GRID_IMAGES2}>
            <section className="space-y-6">
              <div className="mx-auto w-full max-w-7xl">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {GRID_IMAGES2.map((src, i) => (
                    <button key={src} type="button" onClick={() => openLightbox(src, "Detail study.")} className="block w-full">
                      <div className={i < 4 ? "aspect-6/4 w-full overflow-hidden" : "aspect-4/6-full overflow-hidden"}>
                        <img src={src} alt="Detail" className="block w-full h-full object-cover" />
                      </div>
                    </button>
                  ))}
