// src/components/StarfieldButton.tsx
...
type Colors = {
    ...
    /** Opacity (0–100) applied on top of the fill color's own alpha — lets the
     *  face be dialed transparent without having to hand-encode alpha into
     *  the color string itself. */
    fillOpacity?: number
}
...
function __OriginkitBase_StarfieldButton(props: StarfieldButtonProps) {
    const {
        ...
        pixel = { size: .5, color: "#FC731C", density: 100, brightness: 100 },
        link = "",
    ...
    // The face paints Fill as given, then Fill Opacity multiplies on top of
    // whatever alpha the color string already carries — a separate dial
    // rather than requiring an rgba()/#rrggbbaa string to dial it down.
    const fillOpacityPct = Math.max(0, Math.min(100, colors?.fillOpacity ?? 100)) / 100
    const fillRGBA = parseColor(fill)
    const faceBackground = rgba(fillRGBA, fillRGBA.a * fillOpacityPct)
