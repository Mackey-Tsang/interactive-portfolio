...
export interface MorphSliderProps {
  ...
  className?: string;
  /** Fires whenever the slider's active index changes — from its own UI
   *  (arrows / dots / drag) or from an external next()/prev() call made
   *  through the ref. Lets a parent keep its own state in sync. */
  onIndexChange?: (index: number) => void;
  [key: string]: unknown;
}

/** Imperative handle so a parent can drive the slider externally (e.g. from
 *  its own drag/scroll gesture) without going through the slider's own
 *  built-in pointer/keyboard/UI controls. */
export interface MorphSliderHandle {
  next: () => void;
  prev: () => void;
  /** Jump directly by any signed offset (not just ±1) — e.g. for a dot
   *  indicator that lets you click straight to project 5 from project 1. */
  ...
  private loadTextures(): void {
    this.items.forEach((item, index) => {
      ...
      img.onerror = () => {
        ...
        console.error(
          `[MorphSlider] Failed to load image at index ${index}: "${item.image}". ` +
            `Check that the path is correct and the file actually exists under your Next.js "public" folder ` +
...
function MorphSliderInner(
  ...
  ref: React.Ref<MorphSliderHandle>
) {
  ...
  // External control — lets a parent drive the slider (e.g. from its own
  // drag/scroll gesture) via a ref, without touching the slider's own UI.
  useImperativeHandle(
    ref,
    () => ({
    ...
    >
      ...
      {showControls && (
        <div className="absolute top-1/2 left-0 right-0 z-3 flex justify-between px-4 -translate-y-1/2 pointer-events-none">
          <button
            type="button"
            className="pointer-events-auto inline-flex items-center justify-center w-10 h-10 rounded-full text-white border border-white/20 bg-[rgba(12,12,14,0.4)] backdrop-blur-md cursor-pointer transition-transform duration-200 hover:scale-105 hover:bg-[rgba(24,24,28,0.6)] active:scale-95 focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-white/80"
        ...
        >
          {items.map((item, i) => (
            <button
              ...
              className={`h-2 rounded-full cursor-pointer [transition:width_var(--ms-dot)_cubic-bezier(0.16,1,0.3,1),background-color_var(--ms-dot)_ease] focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-white/80 ${
...
const MorphSlider = forwardRef(MorphSliderInner);
